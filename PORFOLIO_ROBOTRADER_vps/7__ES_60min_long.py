...
data = ib.reqHistoricalData(
    contract,
    endDateTime='',
    durationStr='2 D', # la duracion del historico requerido dependera del mayor de los parametros de los indicadores usados en la estrategia
...
def on_new_bar(bars: BarDataList, has_new_bar: bool): 
    if has_new_bar: # cada vez que hay nuva barra:
        ...
        # leemos el df_out
        df_out = pd.read_pickle('df_out_7.pkl')
        ...
        if len(df_out) > 1:
            ...
            df_out['ema_6'].iloc[-2] = df['ema_6'].iloc[-2]
