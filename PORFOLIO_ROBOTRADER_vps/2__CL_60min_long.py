...
def on_new_bar(bars: BarDataList, has_new_bar: bool): 
    if has_new_bar: # cada vez que hay nuva barra:
        ...
        df['in_condition_1'] =  (df['close'] > df['open']) & (df['close'].shift() > df['open'].shift())
        df['in_condition_2'] = df['close'] > df['sma_12']
        df['out_condition_1'] = df['close'] < df['sma_60']
        df['out_condition_2'] = df['close'] < (df['close'].shift(2) - 2 * df['atr_14'])
        ...
        # leemos el df_out
        df_out = pd.read_pickle('df_out_2.pkl')
        ...
        # y actualizamos los indicadores
            ...
            df_out['sma_12'].iloc[-2] = df['sma_12'].iloc[-2]
