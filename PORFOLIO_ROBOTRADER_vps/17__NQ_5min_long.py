...
data = ib.reqHistoricalData(
    contract,
    endDateTime='',
    durationStr='4200 S', # la duracion del historico requerido dependera del mayor de los parametros de los indicadores usados en la estrategia
...
def on_new_bar(bars: BarDataList, has_new_bar: bool): 
    if has_new_bar: # cada vez que hay nuva barra:
        ...
        df['out_condition_2'] = df['close'] < (df['close'].shift(2) - 1.3 * df['atr_14'])
        ...
        # leemos el df_out
        df_out = pd.read_pickle('df_out_17.pkl')
        ...
        df_out.to_pickle('df_out_17.pkl')
