...
data = ib.reqHistoricalData(
    contract,
    endDateTime='',
    durationStr='54000 S', # la duracion del historico requerido dependera del mayor de los parametros de los indicadores usados en la estrategia
...
def on_new_bar(bars: BarDataList, has_new_bar: bool): 
    if has_new_bar: # cada vez que hay nuva barra:
        ...
        # añadimos los indicadores de la estrategia
        ...
        df['in_condition_1'] = df['three_bars_down']
        df['in_condition_2'] = df['close'] < df['close'].shift(2) - 2 * df['atr_14']
        df['out_condition_1'] = df['three_bars_up']
        df['out_condition_2'] = df['close'] > df['sma_60']
        ...
        # leemos el df_out
        df_out = pd.read_pickle('df_out_16.pkl')
        ...
        df_out.to_pickle('df_out_16.pkl')
