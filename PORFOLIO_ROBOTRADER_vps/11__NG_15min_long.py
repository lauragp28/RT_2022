...
def on_new_bar(bars: BarDataList, has_new_bar: bool): 
    if has_new_bar: # cada vez que hay nuva barra:
        ...
        df['three_bars_up'] = (df['close'] > df['open']) & (df['close'].shift() > df['open'].shift()) & (df['close'].shift(2) > df['open'].shift(2))
        ...
        df['in_condition_1'] = df['three_bars_up']
        df['in_condition_2'] = df['close'] > df['close'].shift(2) + 2 * df['atr_200']
        df['out_condition_1'] = df['close'] < df['bb_lowband_20__2'] 
        df['out_condition_2'] = df['close'] < df['min_200']
        ...
        # leemos el df_out
        df_out = pd.read_pickle('df_out_11.pkl')
        ...
        if len(df_out) > 1:
            ...
            df_out['three_bars_up'].iloc[-2] = df['three_bars_up'].iloc[-2]
            df_out['atr_200'].iloc[-2] = df['atr_200'].iloc[-2]
            df_out['bb_lowband_20__2'].iloc[-2] = df['bb_lowband_20__2'].iloc[-2]
            df_out['min_200'].iloc[-2] = df['min_200'].iloc[-2]
        ...
        df_out.to_pickle('df_out_11.pkl')
