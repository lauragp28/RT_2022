...
def on_new_bar(bars: BarDataList, has_new_bar: bool): 
    if has_new_bar: # cada vez que hay nuva barra:
        ...
        df['min_12'] = df['close'].rolling(window=12, min_periods=1).min()

        # añadimos las condiciones
        df['in_condition_1'] = df['close'] > df['close'].shift(2) + 1.3 * df['atr_14']
        df['in_condition_2'] = df['close'] > df['close'].shift(2) + 2 * df['atr_200']
        df['out_condition_1'] = df['close'] < df['min_12']
        df['out_condition_2'] = df['close'] < (df['close'].shift(2) - 2 * df['atr_200'])
        ...
        # leemos el df_out
        df_out = pd.read_pickle('df_out_9.pkl')
        ...
        if len(df_out) > 1:
            ...
            df_out['min_12'].iloc[-2] = df['min_12'].iloc[-2]
