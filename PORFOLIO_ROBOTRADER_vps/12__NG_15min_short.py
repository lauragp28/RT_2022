...
def on_new_bar(bars: BarDataList, has_new_bar: bool): 
    if has_new_bar: # cada vez que hay nuva barra:
        ...
        df['max_12'] = df['close'].rolling(window=12, min_periods=1).max()
        ...
        df['out_condition_1'] = df['close'] > df['max_12']
        ...
        # leemos el df_out
        df_out = pd.read_pickle('df_out_12.pkl')
        ...
        if len(df_out) > 1:
            ...
            df_out['max_12'].iloc[-2] = df['max_12'].iloc[-2]
        ...
        df_out.to_pickle('df_out_12.pkl')
